...
class NgramModel(object):
    ...
    def prob(self, event, context):
        ...
        # Laplace smoothing, if possible
        try:
            smoothing = 1.0 / self._estimators[context].N()
        except KeyError:
            # If the context doesn't even exist, it's difficult to come
            # up with a valid estimate. The best answer is 1.0.
            return 1.0
        try:
            return smoothing + self._estimators[context].freq(event)
        except KeyError:
            return smoothing
...
def _make_ngram_tuples(samples, n):
    ...
    if n == 1:
        ...
        # list of the tuples
        # TODO: Rewrite using islice and make this whole thing a generator
        ngrams = []
